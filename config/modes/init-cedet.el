...
;; 增加头文件解析路径
(defconst VA-include-dirs
  (list "~/VA/src")
  )
(defconst W32-include-dirs
  (list "C:/Program Files (x86)/Microsoft Visual Studio/VC98/MFC/Include")
  )
(require 'semantic-c nil 'noerror)
(let ((include-dirs  VA-include-dirs))
  (when (eq system-type 'windows-nt)
    (setq include-dirs (append include-dirs  W32-include-dirs))
    )
  (mapc (lambda (dir)
